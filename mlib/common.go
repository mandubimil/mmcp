...
func GetConfig() (map[string]interface{}, error) {
	dat, err := ioutil.ReadFile("../mmcp.conf")
	CheckErr(err)
...
// ExePythonStock hhh
func ExePythonStock(p_formData map[string][]string) (string, error) {
	resp, err := http.PostForm("http://192.168.0.112:1204/python_exe", p_formData)
	CheckErr(err)
